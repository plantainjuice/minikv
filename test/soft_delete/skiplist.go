...
func (list SkipList) HasNode(Key Key) *Node {
	if list.Level >= 0 {
		level := list.Level
...
// 删除节点
func (list *SkipList) DeleteNode(Key Key) {
	//todo 如果顶层只有一个节点删除后会不会异常？
	node := list.HasNode(Key)
	if node == nil {
		return
	}
	for node != nil {
...
// 添加数据到跳表中
func (list *SkipList) AddNode(Key Key) {
	// 如果包含相同的数据，就返回，不用添加了
	if list.HasNode(Key) != nil {
		return
	}
	...
	list.InsertValue(Key, headNodeInsertPositionArr)
}

func (list *SkipList) InsertValue(Key Key, headNodeInsertPositionArr []*Node) {
	// 插入最底层
	node := new(Node)
	node.Key = Key
	if list.Level < 0 {
